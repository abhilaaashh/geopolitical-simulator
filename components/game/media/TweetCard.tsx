    ...
    <div className="flex gap-3 py-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1 flex-wrap">
          <span className="font-semibold text-white text-sm">{reply.author.displayName}</span>
          ...
          {reply.isPopular && (
            <span className="text-xs bg-blue-500/20 text-blue-400 px-1.5 py-0.5 rounded ml-1">
              Top reply
            </span>
          )}
        </div>
        <p className="text-gray-200 text-sm mt-1 whitespace-pre-wrap">{reply.content}</p>
        <div className="flex items-center gap-4 mt-2">
          <button className="flex items-center gap-1 text-gray-500 hover:text-blue-400 text-xs">
            <MessageCircle className="w-3.5 h-3.5" />
          </button>
          <button className="flex items-center gap-1 text-gray-500 hover:text-green-400 text-xs">
            <Repeat2 className="w-3.5 h-3.5" />
          </button>
          <button className="flex items-center gap-1 text-gray-500 hover:text-pink-400 text-xs">
...
export function TweetCard({ event, actorColor }: TweetCardProps) {
  ...
  return (
    <motion.div
      ...
      className={`${styles.bg} rounded-2xl border border-gray-800 overflow-hidden ${
    ...
    >
      ...
      <div className="px-4 py-2 border-b border-gray-800 flex items-center justify-between">
        <span className={`text-lg font-bold ${styles.accent}`}>{styles.logo}</span>
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-gray-500">{styles.name}</span>
        </div>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            {/* Name and handle */}
            <div className="flex items-center gap-1 flex-wrap">
              <span className="font-bold text-white truncate">{event.actorName}</span>
              {verified && (
                <BadgeCheck className={`w-5 h-5 ${isNews ? 'text-yellow-500' : styles.accent} flex-shrink-0`} />
              )}
              {isPlayer && (
                <span className="text-xs bg-game-accent/20 text-game-accent px-2 py-0.5 rounded-full ml-1">
                  You
                </span>
              )}
              {isPublicUser && (
        ...
        {/* Tweet text */}
        <div className="mt-3 text-white text-[15px] leading-relaxed whitespace-pre-wrap">
          {event.content}
        </div>
        ...
        {event.media?.quoteTweet && (
          <div className="mt-3 border border-gray-700 rounded-xl p-3">
            ...
            <p className="text-gray-300 text-sm">{event.media.quoteTweet.content}</p>
          </div>
        )}
        ...
        {/* Engagement stats */}
        <div className="mt-3 pt-3 border-t border-gray-800 flex items-center gap-1 text-sm text-gray-500">
        ...
        {/* Action buttons */}
        <div className="mt-3 pt-3 border-t border-gray-800 flex items-center justify-around">
          <button 
